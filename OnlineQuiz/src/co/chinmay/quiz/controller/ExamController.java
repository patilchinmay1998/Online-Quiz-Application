package co.chinmay.quiz.controller;
...
import co.chinmay.quiz.Exam;
import co.chinmay.quiz.QuizQuestion;
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			if (session.getAttribute("currentExam") == null) {
				...
				String selectedExam = (String) request.getSession()
						.getAttribute("exam");
		...
		if ("Next".equals(action)) {
			...
			request.getRequestDispatcher("/WEB-INF/jsps/result.jsp").forward(
					request, response);

		}

		if (finish != true) {
			request.getRequestDispatcher("/WEB-INF/jsps/exam.jsp").forward(
					request, response);
		}
